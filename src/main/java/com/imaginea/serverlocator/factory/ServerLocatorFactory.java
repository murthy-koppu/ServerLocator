...
public class ServerLocatorFactory {
	...
	public static ServerProperties getServerLocator(ServersEnum[] optimizedChoices,InetAddress iNetAddr, int port) {	
		log.info("*********** Inside ServerLocatorFactory -> getServerLocator( ) **********");
		log.debug("Optimized Choice provided "+ optimizedChoices == null ? "Empty" : Arrays.toString(optimizedChoices));
		log.debug("Provided HostAddress is "+iNetAddr.toString()+" and port number is "+port);
		ServerProperties serverProp = new ServerProperties();
		Socket clientSocket = null;
		try{
			clientSocket = new Socket(iNetAddr, port);	
		}catch (IOException e) {
			log.debug("Unable to connect to server with provided host and port details", e);
		...
		log.debug("Identified Server Status as "+serverProp.getConnectionStatus());
	...
	private static ServerProperties chooseServerLocatorFmChoices(
		...
		log.debug("Optimized Choice selected "+ Arrays.toString(optimizedChoices));
		for(int i = 0; i < 2; i++){
			for(ServersEnum inServerChoice: optimizedChoices){
				...
				if(serverProp != null){
					...
					log.debug(" At HostName "+serverProp.getHostName()+" on port "+serverProp.getPortNo());
					log.debug("Found Server Details with Server Name "+ serverProp.getServerName()+" and version "+serverProp.getVersion());					
	...
	private static ServerProperties findServerDetails(InetAddress iNetAddr, int port,
		...
		log.debug("*** Inside ServerLocatorFactory -> findServerDetails( ) with selected Choice "
			+ inServerChoice.toString()+" and limitedTimeOut field as "+isLimitedTimeOut+" ***");
